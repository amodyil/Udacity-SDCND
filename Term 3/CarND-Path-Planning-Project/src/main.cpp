		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2')
		{
			...
			if (s != "")
			{
				...
				if (event == "telemetry")
				{
					...
					if (car_ahead)
					{
						if (!car_left && lane > LEFT)
							lane--; // move left
						else if (!car_right && lane != CENTER)
							lane++; // move right
						else
							speed_diff -= MAX_ACC; // slow down
					}
					else
					{
						if (lane != CENTER)
							if ((lane == LEFT && !car_right) || (lane == RIGHT && !car_left))
								lane = CENTER; // move to center lane
					...
					// Start with all of the the points from previous path points from last time
